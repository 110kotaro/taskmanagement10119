...
interface Task {
  ...
  priority: PriorityLabel; // 'important' | 'normal' | 'low' | 'none'
  customPriority?: string;
  taskType: TaskType; // 'normal' | 'meeting' | 'regular' | 'project' | 'other'
  memo?: string;
  files?: Array<{id, name, url, uploadedAt}>;
  subtasks: SubTask[];
...
- フィルター機能：
  ...
  - 期限切れ（独立したチェックボックス）
  - 優先度（複数選択可能）：重要、普通、低め、なし
  - タスクタイプ（複数選択可能）：通常、会議、定期、プロジェクト、その他
- ソート機能：
  - 終了日、作成日、優先度、タイトル、完了日時
  - 昇順/降順の切り替え
- 一括削除機能
- 期限切れタスクの背景色：
  - 開始日が過ぎている：黄色背景
...
- 個人モードの場合：チームプロジェクトで、そのプロジェクト内に自分が担当しているタスクがある場合
...
- 自分が作成したプロジェクトのみ表示
- 自分が所属しているプロジェクトを表示
- チームプロジェクトで、そのプロジェクト内に自分が担当しているタスクがある場合のみ表示
...
- `ProjectMemberRoleChanged`: プロジェクトメンバー権限変更
